...
void arp_spoof_request(struct libnet_link_int *iface, char *dev, 
...
int main(int argc, char **argv)
{
    ...
    if(argc < 5)
    {
	printf("usage: %s <src IP> <src hw> <tgt ip> <tgt hw>\n", argv[0]);
	exit(1);
    }
    ...
    src.ip = libnet_name_resolve(argv[1], 0);
    memcpy(src.eth, hex2eth(argv[2]), ETH_ALEN);

    dst.ip = libnet_name_resolve(argv[3], 0);
    memcpy(dst.eth, hex2eth(argv[4]), ETH_ALEN);
    
    if((pd = pcap_open_live(dev, ETH_MTU, 1, 0, NULL)) == NULL)
    {
	perror("pcap_open");
	exit(1);
    }
    ...
    while(1)
    {
	fr = (struct tcpframe *) pcap_next(pd, &h);
	if(((struct libnet_ip_hdr *)fr->ip)->ip_p == IPPROTO_ICMP)
	{
	    if(!memcmp(&((struct libnet_ip_hdr *)fr->ip)->ip_src, &src.ip, 4))
	    {
		printf("Got our own packet back! Bad!!!\n");
	    }
	    if(!memcmp(&((struct libnet_ip_hdr *)fr->ip)->ip_dst,
			&src.ip, 4))
	    {
		printf("Got reply for our own packet back! Good!\n");
	    }
	}
    }
#endif 
